...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss)
{
    ...
    for (auto vShareDamageAura : vShareDamageAuras)
    {
        ...
        {
            if (shareTarget != pVictim && (vShareDamageAura->GetMiscValue() & damageSchoolMask))
            {
    ...
    if (pVictim->GetTypeId() == TYPEID_PLAYER)
        static_cast<Player*>(pVictim)->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_RECEIVED, damage);

    if (pVictim->GetTypeId() == TYPEID_UNIT && !((Creature*)pVictim)->IsPet() && !((Creature*)pVictim)->HasLootRecipient())
        ((Creature*)pVictim)->SetLootRecipient(this);

    if (pVictim->GetTypeId() == TYPEID_PLAYER)
        static_cast<Player*>(pVictim)->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_TOTAL_DAMAGE_RECEIVED, damage);
        ...
        if (spellProto->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && spellProto->SpellFamilyFlags & uint64(0x0000000400000000))
        {
            ...
            if (found)
            {
                ...
                for (auto dummyAura : dummyAuras)
                {
                    if (dummyAura->GetSpellProto()->EffectMiscValue[dummyAura->GetEffIndex()] == 7244)
                    {
                        DonePercent *= (dummyAura->GetModifier()->m_amount + 100.0f) / 100.0f;
                        break;
                    }
                }
            }
...
uint32 Unit::MeleeDamageBonusTaken(Unit* pCaster, uint32 pdamage, WeaponAttackType attType, SpellSchoolMask schoolMask, SpellEntry const* spellProto, DamageEffectType damagetype, uint32 stack, bool flat)
{
    ...
    for (auto mDummyAura : mDummyAuras)
    {
        switch (mDummyAura->GetId())
        {
            ...
            case 20911:                                     // Blessing of Sanctuary
            case 25899:                                     // Greater Blessing of Sanctuary
                TakenPercent *= (mDummyAura->GetModifier()->m_amount + 100.0f) / 100.0f;
                break;
        }
    }
